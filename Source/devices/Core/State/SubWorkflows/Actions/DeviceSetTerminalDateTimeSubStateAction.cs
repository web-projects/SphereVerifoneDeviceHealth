...
{
    internal class DeviceSetTerminalDateTimeSubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => DeviceSubWorkflowState.SetTerminalDateTime;

        public DeviceSetTerminalDateTimeSubStateAction(IDeviceSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                //_ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to update device with timestamp.");
                Console.WriteLine("Unable to find a state object while attempting to update device with timestamp.");
                _ = Error(this);
            }
            else
            {
                ...
                foreach (var device in Controller.TargetDevices)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => device.SetTerminalDateTime(devicesRequest.Last()),
                        DeviceConstants.CardCaptureTimeout,
                        System.Threading.CancellationToken.None);

                    if (timeoutPolicy.Outcome == Polly.OutcomeType.Failure)
                    {
                        //_ = Controller.LoggingClient.LogErrorAsync($"Unable to update device with timestamp - '{Controller.DeviceEvent}'.");
                        Console.WriteLine($"Unable to update device with timestamp - '{Controller.DeviceEvent}'.");
