...
{
    internal class DeviceUnlockConfigurationSubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => DeviceSubWorkflowState.UnlockDeviceConfig;

        public DeviceUnlockConfigurationSubStateAction(IDeviceSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                //_ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to unlock device configuration.");
                Console.WriteLine("Unable to find a state object while attempting to unlock device configuration.");
                _ = Error(this);
            }
            else
            {
                ...
                foreach (var device in Controller.TargetDevices)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => device.UnlockDeviceConfiguration(devicesRequest.Last()),
                        DeviceConstants.CardCaptureTimeout,
                        System.Threading.CancellationToken.None);

                    if (timeoutPolicy.Outcome == Polly.OutcomeType.Failure)
                    {
                        //_ = Controller.LoggingClient.LogErrorAsync($"Unable to unlock device configuration - '{Controller.DeviceEvent}'.");
                        Console.WriteLine($"Unable to unlock device configuration - '{Controller.DeviceEvent}'.");
