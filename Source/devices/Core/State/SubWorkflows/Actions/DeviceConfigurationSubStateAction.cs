...
{
    internal class DeviceConfigurationSubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => DeviceSubWorkflowState.Configuration;

        public DeviceConfigurationSubStateAction(IDeviceSubStateController _) : base(_) { }
            ...
            else
            {
                ...
                foreach (var device in Controller.TargetDevices)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => device.Configuration(devicesRequest.Last()),
