...
{
    internal class DeviceRebootSubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => RebootDevice;

        public DeviceRebootSubStateAction(IDeviceSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                //_ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to call reboot device.");
                Console.WriteLine("Unable to find a state object while attempting to call reboot device.");
                _ = Error(this);
            }
            else
            {
                ...
                if (cardDevice != null)
                {
                    ...
                    _ => cardDevice.RebootDevice(linkRequest),
                    DeviceConstants.CardCaptureTimeout,
                    this.CancellationToken);

                    if (timeoutPolicy.Outcome == Polly.OutcomeType.Failure)
                    {
                        //_ = Controller.LoggingClient.LogErrorAsync($"Unable to process Reboot request from device - '{Controller.DeviceEvent}'.");
                        Console.WriteLine($"Unable to process Reboot request from device - '{Controller.DeviceEvent}'.");
