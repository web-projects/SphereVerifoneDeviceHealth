...
{
    internal class DeviceReboot24HourSubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => DeviceSubWorkflowState.Reboot24Hour;

        public DeviceReboot24HourSubStateAction(IDeviceSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                //_ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to update device with 24 Hour Reboot timestamp.");
                Console.WriteLine("Unable to find a state object while attempting to update device with 24 Hour Reboot timestamp.");
                _ = Error(this);
            }
            else
            {
                ...
                foreach (var device in Controller.TargetDevices)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => device.Reboot24Hour(devicesRequest.Last()),
                        DeviceConstants.CardCaptureTimeout,
                        System.Threading.CancellationToken.None);

                    if (timeoutPolicy.Outcome == Polly.OutcomeType.Failure)
                    {
                        //_ = Controller.LoggingClient.LogErrorAsync($"Unable to update device with 24 Hour Reboot timestamp - '{Controller.DeviceEvent}'.");
                        Console.WriteLine($"Unable to update device with 24 Hour Reboot timestamp - '{Controller.DeviceEvent}'.");
