...
{
    internal class DeviceGetEMVKernelChecksumSubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => DeviceSubWorkflowState.GetEMVKernelChecksum;

        public DeviceGetEMVKernelChecksumSubStateAction(IDeviceSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                //_ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to get EMV kernel checksum.");
                Console.WriteLine("Unable to find a state object while attempting to get EMV kernel checksum.");
                _ = Error(this);
            }
            else
            {
                ...
                foreach (var device in Controller.TargetDevices)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => device.GetEMVKernelChecksum(devicesRequest.Last()),
                        DeviceConstants.CardCaptureTimeout,
                        System.Threading.CancellationToken.None);

                    if (timeoutPolicy.Outcome == Polly.OutcomeType.Failure)
                    {
                        //_ = Controller.LoggingClient.LogErrorAsync($"Unable to obtain EMV kernel checksum - '{Controller.DeviceEvent}'.");
                        Console.WriteLine($"Unable to obtain EMV kernel checksum - '{Controller.DeviceEvent}'.");
