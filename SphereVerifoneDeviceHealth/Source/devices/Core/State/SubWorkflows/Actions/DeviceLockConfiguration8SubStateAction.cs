...
{
    internal class DeviceLockConfiguration8SubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => DeviceSubWorkflowState.LockDeviceConfig8;

        public DeviceLockConfiguration8SubStateAction(IDeviceSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                //_ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to lock device configuration 8.");
                Console.WriteLine("Unable to find a state object while attempting to lock device configuration 8.");
                _ = Error(this);
            }
            else
            {
                ...
                foreach (var device in Controller.TargetDevices)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => device.LockDeviceConfiguration8(devicesRequest.Last()),
                        DeviceConstants.CardCaptureTimeout,
                        System.Threading.CancellationToken.None);

                    if (timeoutPolicy.Outcome == Polly.OutcomeType.Failure)
                    {
                        //_ = Controller.LoggingClient.LogErrorAsync($"Unable to lock device configuration 8 - '{Controller.DeviceEvent}'.");
                        Console.WriteLine($"Unable to lock device configuration 8 - '{Controller.DeviceEvent}'.");
